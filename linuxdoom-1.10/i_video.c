...
void I_ShutdownGraphics(void) { SDL_Quit(); }
...
void I_GetEvent(SDL_Event *Event) {
    ...
    switch (Event->type) {
        ...
        case SDL_MOUSEBUTTONUP:
            ...
            event.type = ev_mouse;
            event.data1 = 0 | (buttonstate & SDL_BUTTON(1) ? 1 : 0) | (buttonstate & SDL_BUTTON(2) ? 2 : 0) |
...
void I_FinishUpdate(void) {
    ...
    if ((multiply == 1) && SDL_MUSTLOCK(screen)) {
        ...
        for (i = 0; i < 2; i++) {
            olineptrs[i] = (unsigned int *) &((Uint8 *) screen->pixels)[i * screen->pitch];
        }
...
void I_InitGraphics(void) {
    ...
    if (doShm) {
        ...
        image = XCreateImage(X_display, X_visual, 8, ZPixmap, 0, (char *) malloc(X_width * X_height), X_width, X_height,
                             8, X_width);
    }
